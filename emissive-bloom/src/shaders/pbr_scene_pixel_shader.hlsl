...
PSOutput PS(PSInput input)
{
    ...
    if (displayMode == 1 || displayMode == 2)
    {
        ...
        if (displayMode == 1)
        {
            ...
            output.sceneColor = float4(distributionValue.xxx, (alphaMode == 2) ? alpha : 1.0f);
            return output;
        }
        ...
        output.sceneColor = float4(geometryValue.xxx, (alphaMode == 2) ? alpha : 1.0f);
        return output;
    }
    ...
    if (displayMode == 3)
    {
        output.sceneColor = float4(clamp(accumulatedFresnel, 0.0f.xxx, 1.0f.xxx), (alphaMode == 2) ? alpha : 1.0f);
        return output;
    }
